...
class CompositionLayoutRenderTrait:
    ...
    def layout_immediate(self, canvas_size: Geometry.IntSize, force: bool=True) -> None:
        ...
        self._canvas_item_composition.update_layout(Geometry.IntPoint(), canvas_size, immediate=True)
    ...
    def _update_layout(self, canvas_origin: typing.Optional[Geometry.IntPoint],
        ...
        with self.__layout_lock:
            if self.__canvas_items is not None:
                ...
                super().update_layout(canvas_origin_, canvas_size_, immediate=immediate)
...
class ScrollAreaLayout(CanvasItemLayout):
    ...
    def layout(self, canvas_origin: Geometry.IntPoint, canvas_size: Geometry.IntSize, canvas_items: typing.Sequence[LayoutItem], *, immediate: bool = False) -> None:
        ...
        if content and (self.needs_layout or not content._has_layout):
            # only update the layout if the content has not layout yet. the content is free to update
            # as needed from now on without being affected by the scroll area size.
...
class ScrollAreaCanvasItem(CanvasItemComposition):
    ...
    def __handle_canvas_size_changed(self, canvas_size: typing.Optional[Geometry.IntSizeTuple]) -> None:
        self.__scroll_area_layout.needs_layout = True
...
class SplitterLayout(CanvasItemLayout):
    ...
    def sizings(self) -> typing.Sequence[Sizing]:
        with self.__lock:
            return copy.copy(self.__sizings)
...
class SplitterCanvasItem(CanvasItemComposition):
    ...
    def __init__(self, orientation: typing.Optional[str] = None) -> None:
        ...
        self.__tracking_sizings: typing.List[Sizing] = []
    ...
    def mouse_pressed(self, x: int, y: int, modifiers: UserInterface.KeyboardModifiers) -> bool:
        ...
        if index > 0:
            ...
            self.__tracking_sizings = sizings
    ...
    def mouse_released(self, x: int, y: int, modifiers: UserInterface.KeyboardModifiers) -> bool:
        ...
        for index, (old_sizing, current_sizing) in enumerate(zip(self.__tracking_sizings, sizings)):
            sizing = Sizing()
            sizing._copy_from(old_sizing)
            if index == self.__tracking_start_index or index == self.__tracking_start_index + 1:
                if self.orientation == "horizontal":
                    sizing._preferred_height = current_sizing.preferred_height
                else:
                    sizing._preferred_width = current_sizing.preferred_width
